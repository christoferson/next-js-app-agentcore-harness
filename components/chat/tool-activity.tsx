...
function previewInput(input: string): { text: string; pretty: boolean } {
...
function ToolChip({ tool }: { tool: ToolRecord }) {
  ...
  return (
    <div className="rounded-lg border border-border bg-muted/30">
      ...
      {expanded && text && (
        <pre className="max-h-48 overflow-auto border-t border-border bg-black/30 px-2.5 py-2 font-mono text-[11px] leading-relaxed text-slate-300">
          {text}
        </pre>
      )}
    </div>
...
function ToolLive({ tool }: { tool: ToolRecord }) {
  ...
  return (
    <div className="rounded-lg border border-blue-500/30 bg-blue-500/5">
      ...
      {text && (
        <pre className="max-h-40 overflow-auto border-t border-blue-500/20 bg-black/30 px-2.5 py-2 font-mono text-[11px] leading-relaxed text-slate-300">
          {text}
        </pre>
      )}
    </div>
